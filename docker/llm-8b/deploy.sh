...
# google/gemma-4-E4B-it LLM 컨테이너를 띄우는 헬퍼 스크립트 (8B 변형).
#
# 전제 (사전 1회 셋업):
...
case "$cmd" in
    ...
    up)
        ...
        echo "   상태:   $0 ps"
        echo
        echo "⚠️  첫 기동 시 모델(gemma-4-E4B) 다운로드에 시간 걸림."
    ...
    clean)
        require_context
        echo "⚠️  컨테이너 + 볼륨(모델 캐시 dais_llm_8b_hf_cache) 모두 삭제합니다."
